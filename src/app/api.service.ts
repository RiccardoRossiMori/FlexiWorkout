...
export class ApiService implements OnInit {
  private apiUrl = 'https://api-ninjas.com/api/exercises';
  private user: firebase.User | null = null;
  private userId: string = '';


  constructor(
    //@Inject(API_HTTP_CLIENT) private http: ApiInterceptor
  ...
  //   //     "checked": false
  //   //   }
  //   // ]; //httpClient.get<any>(this.apiUrl + str, {headers});*/
  // }

  pushExercises(esercizi: EsercizioAPI[]) {
    // Recupera l'utente e verifica che sia loggato
    if (!this.user) {
    ...
    for (let i = 0; i < esercizi.length; i++) {
      ...
      docRef.set(JSON.parse(gEsercizio))
        .then(() => {
          console.log(`Documento ${documentName} salvato correttamente!`);
        })
        .catch((error) => {
          console.error(`Errore durante il salvataggio del documento ${documentName}:`, error);
        });
    }
  }
}
